...
process_mail <- function(fetch_uid) {
  #fetch_uid = "45"
  ...
  # convert to the same names as email log
  email_UID <- fetch_uid
  replied <- ret
  m <- data.frame(timestamp, email_UID, subject, from, council, processed, 
                  attachment_name, replied)
...
update_submit_log <- function(emails) {
  t <- "logs/email_log.xlsx"
  wb <- loadWorkbook(t)
